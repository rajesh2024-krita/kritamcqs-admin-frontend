...
function AffiliatesTab({ affiliates, filters, page, onAccess, onCopy, onDelete, onDetails, onEdit, onFilter, onForceLogout, onNotify, onRegenerate, onStatus }) {
  return <div className="space-y-4"><SearchFilters filters={filters} onFilter={onFilter} fields={["search", "status"]} /><Table headers={["Affiliate", "Code", "Contact", "Status", "Cycle", "Purchases", "Earnings", "Pending", "Actions"]} rows={affiliates.map((item) => [<button onClick={() => onDetails(item)} className="text-left font-black text-indigo-700">{item.affiliateName}</button>, item.affiliateCode, <span>{item.email}<br/>{item.mobile || "-"}</span>, <select value={item.status} onChange={(event) => onStatus(item, event.target.value)} className="rounded-lg border px-2 py-1 text-xs font-bold"><option>ACTIVE</option><option>INACTIVE</option><option>SUSPENDED</option><option>DELETED</option></select>, `Cycle ${item.currentCycle || 1}`, item.successfulPurchases, money(item.commissionEarned), money(item.pendingPayment), <div className="flex flex-wrap gap-1"><IconAction title="View" onClick={() => onDetails(item)} icon={Eye} /><IconAction title="Edit" onClick={() => onEdit(item)} icon={Pencil} /><button onClick={() => onAccess(item)} className="rounded-lg border px-2 text-xs font-bold">Access</button><button onClick={() => onForceLogout(item)} className="rounded-lg border px-2 text-xs">Logout</button><button onClick={() => onRegenerate(item)} className="rounded-lg border px-2 text-xs">New link</button><IconAction title="Copy link" onClick={() => onCopy(item.referralLink)} icon={Copy} /><IconAction title="Notify" onClick={() => onNotify(item)} icon={Send} />{item.status !== "DELETED" && <IconAction title="Delete" onClick={() => onDelete(item)} icon={Trash2} danger />}</div>])} /><Pagination page={page} onPage={(next) => onFilter({ ...filters, page: next })} /></div>;
}
...
function PaymentCyclesTab({ rows, onStatus }) { const statuses = ["ACTIVE", "TARGET_REACHED", "ELIGIBLE_FOR_PAYMENT", "ADMIN_REVIEW", "PAYMENT_PROCESSING", "PAYMENT_SENT", "PAYMENT_COMPLETED", "FAILED", "CANCELLED"]; return <Table headers={["Affiliate", "Cycle", "Progress", "Purchase value", "Payable", "Status", "Method / Reference", "Payment date", "Last updated"]} rows={rows.map((item) => [item.affiliateId?.affiliateName, `Cycle ${item.cycleNumber}`, `${item.successfulReferralCount} / ${item.targetCount}`, money(item.totalPurchaseValue), money(item.finalPayableAmount ?? item.earnings), <select value={item.status} onChange={(event) => onStatus(item, event.target.value)} className="rounded-lg border p-2 text-xs font-bold">{statuses.map((status) => <option key={status}>{status}</option>)}</select>, `${item.paymentMethod || "-"} / ${item.transactionReference || "-"}`, date(item.paymentDate), date(item.updatedAt)])} />; }
