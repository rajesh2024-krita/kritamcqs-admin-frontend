...
function Settings({ settings, onSave }) {
  ...
  return <form className="grid max-w-2xl gap-4 rounded-2xl border bg-white p-6 shadow-sm" onSubmit={submit}><label className="text-sm font-bold">Referral base URL<input name="referralBaseUrl" defaultValue={settings.referralBaseUrl} className="mt-1 w-full rounded-lg border p-2"/></label><label className="text-sm font-bold">Attribution window days<input name="attributionWindowDays" type="number" defaultValue={settings.attributionWindowDays} className="mt-1 w-full rounded-lg border p-2"/></label><label className="text-sm font-bold">Commission rate percent<input name="commissionRatePercent" type="number" step="0.01" defaultValue={settings.commissionRatePercent} className="mt-1 w-full rounded-lg border p-2"/></label><label className="text-sm font-bold">Purchase milestone thresholds<input name="milestoneThresholds" defaultValue={(settings.milestoneThresholds || [settings.milestoneCount || 25]).join(",")} className="mt-1 w-full rounded-lg border p-2"/><span className="text-xs font-medium text-slate-500">Comma separated, for example: 25,50,100</span></label><button className="w-fit rounded-lg bg-indigo-600 px-4 py-2 font-bold text-white">Save settings</button></form>;
}
