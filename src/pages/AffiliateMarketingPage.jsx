...
const tabs = ["Dashboard", "Affiliates", "Access & Roles", "Journeys", "Purchases", "Payment Cycles", "Communication Center", "Activity", "Settings"];
...
export function AffiliateMarketingPage() {
  ...
  async function load() {
    setLoading(true);
    try {
      const [dashboard, affiliates, referrals, purchases, settings, notifications, adminNotifications, cycles, templates, roles, activities, mailActivity] = await Promise.all([
        ...
        api.roles(),
        api.activities(),
        api.communicationActivity(),
      ]);
      setData({ dashboard: pick(dashboard), affiliates: pick(affiliates), referrals: pick(referrals), purchases: pick(purchases), settings: pick(settings), notifications: pick(notifications), adminNotifications: pick(adminNotifications), cycles: pick(cycles), templates: pick(templates), roles: pick(roles), activities: pick(activities), mailActivity: pick(mailActivity) });
    } catch (error) {
  ...
  async function updateStatus(affiliate, status) {
    try {
      affiliate.status === "DELETED" && status === "ACTIVE" ? await api.restore(affiliate.id) : await api.update(affiliate.id, { status });
  ...
  async function openAccess(affiliate) { try { const response = await api.permissions(affiliate.id); setModal({ mode: "access", affiliate, access: pick(response) }); } catch (error) { toast.error(error.message); } }
  async function saveAccess(event) { event.preventDefault(); const form = event.currentTarget; const overrides = {}; form.querySelectorAll("[data-permission]").forEach((input) => { overrides[input.dataset.permission] = input.checked; }); try { await api.updatePermissions(modal.affiliate.id, { roleId: form.roleId.value, accessEnabled: form.accessEnabled.checked, permissionOverrides: overrides }); toast.success("Access permissions saved"); setModal(null); await load(); } catch (error) { toast.error(error.message); } }
  async function saveRole(event) { event.preventDefault(); const form = event.currentTarget; const permissions = {}; form.querySelectorAll("[data-role-permission]").forEach((input) => { permissions[input.dataset.rolePermission] = input.checked; }); const values = { ...Object.fromEntries(new FormData(form)), permissions }; try { modal.role?.id ? await api.updateRole(modal.role.id, values) : await api.createRole(values); toast.success("Affiliate role saved"); setModal(null); await load(); } catch (error) { toast.error(error.message); } }
    ...
    <div className="space-y-5">
      ...
      {tab === "Affiliates" && (
        <AffiliatesTab
          ...
          onAccess={openAccess}
          onForceLogout={async (affiliate) => { await api.forceLogout(affiliate.id); toast.success("All affiliate sessions revoked"); }}
          onRegenerate={async (affiliate) => { await api.regenerateLink(affiliate.id); toast.success("Referral link regenerated"); await load(); }}
        />
      )}
      ...
      {tab === "Access & Roles" && <RolesTab rows={data.roles || []} onCreate={() => setModal({ mode: "role", role: {} })} onEdit={(role) => setModal({ mode: "role", role })} onDelete={async (role) => { try { await api.deleteRole(role.id); await load(); } catch (error) { toast.error(error.message); } }} />}
      {tab === "Communication Center" && <CommunicationCenter templates={data.templates || []} activity={data.mailActivity?.items || []} notifications={notifications} onCreate={() => setModal({ mode: "template", template: { event: `CUSTOM_EVENT_${Date.now()}`, name: "Custom affiliate event", recipient: "AFFILIATE", notificationEnabled: true, emailEnabled: true, variables: [] } })} onEdit={(template) => setModal({ mode: "template", template })} onPreview={previewTemplate} onRetry={async (item) => { await api.retryCommunication(item.id); await load(); }} />}
      {tab === "Activity" && <SystemActivityTab rows={data.activities?.items || []} />}
      ...
      {modal?.mode === "access" && <AccessModal affiliate={modal.affiliate} access={modal.access} roles={data.roles || []} onClose={() => setModal(null)} onSubmit={saveAccess} />}
      {modal?.mode === "role" && <RoleModal role={modal.role} onClose={() => setModal(null)} onSubmit={saveRole} />}
    </div>
  );
}
...
function AffiliatesTab({ affiliates, filters, page, onAccess, onCopy, onDelete, onDetails, onEdit, onFilter, onForceLogout, onNotify, onRegenerate, onStatus }) {
  return <div className="space-y-4"><SearchFilters filters={filters} onFilter={onFilter} fields={["search", "status"]} /><Table headers={["Affiliate", "Referral Code", "Code Status", "Contact", "Status", "Current Cycle", "Cycle Progress", "Completed Cycles", "Payment Status", "Actions"]} rows={affiliates.map((item) => [<button onClick={() => onDetails(item)} className="text-left font-black text-indigo-700">{item.affiliateName}</button>, item.affiliateCode, item.referralCodeStatus || "ACTIVE", <span>{item.email}<br/>{item.mobile || "-"}</span>, <select value={item.status} onChange={(event) => onStatus(item, event.target.value)} className="rounded-lg border px-2 py-1 text-xs font-bold"><option>ACTIVE</option><option>INACTIVE</option><option>SUSPENDED</option><option>DELETED</option></select>, `Cycle ${item.currentCycle || 1}`, item.currentCycleProgress || "-", item.completedCycles || 0, String(item.paymentStatus || "ACTIVE").replaceAll("_", " "), <div className="flex flex-wrap gap-1"><IconAction title="View" onClick={() => onDetails(item)} icon={Eye} /><IconAction title="Edit" onClick={() => onEdit(item)} icon={Pencil} /><button onClick={() => onAccess(item)} className="rounded-lg border px-2 text-xs font-bold">Access</button><button onClick={() => onForceLogout(item)} className="rounded-lg border px-2 text-xs">Logout</button><button onClick={() => onRegenerate(item)} className="rounded-lg border px-2 text-xs">New link</button><IconAction title="Copy link" onClick={() => onCopy(item.referralLink)} icon={Copy} /><IconAction title="Notify" onClick={() => onNotify(item)} icon={Send} />{item.status !== "DELETED" && <IconAction title="Delete" onClick={() => onDelete(item)} icon={Trash2} danger />}</div>])} /><Pagination page={page} onPage={(next) => onFilter({ ...filters, page: next })} /></div>;
}
...
function PaymentCyclesTab({ rows, onStatus }) { const statuses = ["PAYMENT_PENDING", "PAYMENT_PROCESSING", "PAYMENT_SENT", "PAYMENT_COMPLETED", "FAILED", "CANCELLED"]; return <Table headers={["Affiliate", "Code", "Cycle", "Completed Users", "Commission", "Status", "Method / Reference", "Payment date", "Completed"]} rows={rows.map((item) => [item.affiliateId?.affiliateName, item.affiliateId?.affiliateCode, `Cycle ${item.cycleNumber}`, `${item.successfulReferralCount} / ${item.targetCount}`, money(item.finalPayableAmount ?? item.earnings), <select value={item.status} onChange={(event) => onStatus(item, event.target.value)} className="rounded-lg border p-2 text-xs font-bold">{statuses.map((status) => <option key={status}>{status}</option>)}</select>, `${item.paymentMethod || "-"} / ${item.transactionReference || "-"}`, date(item.paymentDate), date(item.completedAt || item.endDate)])} />; }
...
const permissionLabels = { dashboard: "Dashboard", referrals: "Referral tracking", referralDetails: "Referral details", clickAnalytics: "Click analytics", purchaseDetails: "Purchase details", commissionVisibility: "Commission visibility", currentCycle: "Current payment cycle", paymentHistory: "Payment history", notifications: "Notifications", profile: "Profile", profileEdit: "Profile editing", referralLink: "Referral link", campaigns: "Campaigns", reports: "Download reports", analytics: "Advanced analytics" };
function AccessModal({ affiliate, access, roles, onClose, onSubmit }) { const effective = access.effectivePermissions || {}; return <Modal title={`Access: ${affiliate.affiliateName}`} onClose={onClose}><form onSubmit={onSubmit} className="space-y-4"><label className="flex items-center gap-2 font-bold"><input name="accessEnabled" type="checkbox" defaultChecked={access.accessEnabled}/>Affiliate Panel access enabled</label><label className="block text-sm font-bold">Assigned role<select name="roleId" defaultValue={access.role?.id || ""} className="mt-1 w-full rounded-lg border p-2"><option value="">Default permissions</option>{roles.map((role) => <option key={role.id} value={role.id}>{role.name}</option>)}</select></label><div className="grid gap-2 md:grid-cols-2">{Object.entries(permissionLabels).map(([key, label]) => <label key={key} className="flex items-center gap-2 rounded-lg border p-3 text-sm font-semibold"><input data-permission={key} type="checkbox" defaultChecked={effective[key] !== false}/>{label}</label>)}</div><div className="flex justify-end gap-2"><button type="button" onClick={onClose} className="rounded-lg border px-4 py-2">Cancel</button><button className="rounded-lg bg-indigo-600 px-4 py-2 font-bold text-white">Save access</button></div></form></Modal>; }
function RolesTab({ rows, onCreate, onEdit, onDelete }) { return <div className="space-y-4"><div className="flex items-center justify-between rounded-2xl border bg-white p-5"><div><h2 className="font-black">Reusable affiliate roles</h2><p className="text-sm text-slate-500">Individual overrides take priority over the assigned role, which takes priority over system defaults.</p></div><button onClick={onCreate} className="rounded-lg bg-indigo-600 px-3 py-2 font-bold text-white">Create role</button></div><Table headers={["Role", "Description", "Type", "Enabled permissions", "Actions"]} rows={rows.map((role) => [role.name, role.description, role.isSystem ? "System" : "Custom", Object.entries(role.permissions || {}).filter(([, enabled]) => enabled).map(([key]) => permissionLabels[key] || key).join(", "), <div className="flex gap-2"><button onClick={() => onEdit(role)} className="rounded-lg border px-2 py-1">Edit</button>{!role.isSystem && <button onClick={() => onDelete(role)} className="rounded-lg border px-2 py-1 text-rose-600">Delete</button>}</div>])}/></div>; }
function RoleModal({ role, onClose, onSubmit }) { const permissions = role.permissions || {}; return <Modal title={role.id ? `Edit ${role.name}` : "Create affiliate role"} onClose={onClose}><form onSubmit={onSubmit} className="space-y-4"><label className="block text-sm font-bold">Role name<input name="name" required defaultValue={role.name || ""} className="mt-1 w-full rounded-lg border p-2"/></label><label className="block text-sm font-bold">Description<textarea name="description" defaultValue={role.description || ""} className="mt-1 w-full rounded-lg border p-2"/></label><div className="grid gap-2 md:grid-cols-2">{Object.entries(permissionLabels).map(([key, label]) => <label key={key} className="flex items-center gap-2 rounded-lg border p-3"><input data-role-permission={key} type="checkbox" defaultChecked={permissions[key] !== false}/>{label}</label>)}</div><div className="flex justify-end gap-2"><button type="button" onClick={onClose} className="rounded-lg border px-4 py-2">Cancel</button><button className="rounded-lg bg-indigo-600 px-4 py-2 font-bold text-white">Save role</button></div></form></Modal>; }
function CommunicationCenter({ templates, activity, notifications, onCreate, onEdit, onPreview, onRetry }) { return <div className="space-y-6"><TemplatesTab rows={templates} onCreate={onCreate} onEdit={onEdit} onPreview={onPreview}/><h2 className="text-lg font-black">Email activity and failed deliveries</h2><Table headers={["Recipient", "Affiliate", "Event", "Subject", "Template", "Status", "Sent", "Action"]} rows={activity.map((item) => [item.recipientEmail, item.affiliateId?.affiliateName, item.event, item.subject, item.templateName, item.status, date(item.createdAt), item.status === "FAILED" ? <button onClick={() => onRetry(item)} className="rounded-lg bg-indigo-600 px-3 py-1 text-xs font-bold text-white">Retry</button> : "-"])}/><h2 className="text-lg font-black">Notification activity</h2><NotificationsTab affiliates={[]} filters={{}} page={{ items: notifications, total: notifications.length }} rows={notifications} onFilter={() => {}}/></div>; }
function SystemActivityTab({ rows }) { return <Table headers={["Activity ID", "User type", "Affiliate", "Action", "Module", "Description", "IP address", "Device", "Date"]} rows={rows.map((item) => [item.activityId, item.userType, item.affiliateId?.affiliateName, item.action, item.module, item.description, item.ipAddress, item.device, date(item.createdAt)])}/>; }
...
function Settings({ settings, onSave }) {
  ...
  const Field = ({ label, name, type = "text" }) => <label className="text-sm font-bold">{label}<input name={name} type={type} defaultValue={settings[name] ?? ""} className="mt-1 w-full rounded-lg border p-2"/></label>;
  const Toggle = ({ label, name }) => <label className="text-sm font-bold">{label}<select name={name} defaultValue={String(settings[name] !== false)} className="mt-1 w-full rounded-lg border p-2"><option value="true">Enabled</option><option value="false">Disabled</option></select></label>;
  return <form className="grid gap-5 rounded-2xl border bg-white p-6 shadow-sm md:grid-cols-2" onSubmit={submit}><h2 className="col-span-full text-lg font-black">General configuration</h2><Toggle label="Affiliate registration" name="registrationEnabled"/><Toggle label="Approval required" name="approvalRequired"/><Field label="Affiliate code format" name="affiliateCodeFormat"/><Field label="Referral base URL" name="referralBaseUrl"/><h2 className="col-span-full text-lg font-black">Referral configuration</h2><Field label="Attribution window days" name="attributionWindowDays" type="number"/><Field label="Cookie duration days" name="cookieDurationDays" type="number"/><Toggle label="Existing-user attribution" name="allowExistingUserAttribution"/><Toggle label="Cross-device attribution" name="crossDeviceAttribution"/><Toggle label="App-install attribution" name="appInstallAttribution"/><Toggle label="Duplicate-referral prevention" name="duplicateReferralPrevention"/><label className="text-sm font-bold">Referral data privacy<select name="referralPrivacyMode" defaultValue={settings.referralPrivacyMode || "MASKED"} className="mt-1 w-full rounded-lg border p-2"><option>MASKED</option><option>FULL</option></select></label><h2 className="col-span-full text-lg font-black">Commission and payment</h2><Field label="Default commission rate %" name="commissionRatePercent" type="number"/><Field label="Fixed commission amount" name="fixedCommissionAmount" type="number"/><Toggle label="Commission approval required" name="commissionApprovalRequired"/><Toggle label="Refund commission reversal" name="refundCommissionReversal"/><Field label="Payment cycle target" name="milestoneCount" type="number"/><Field label="Minimum payment threshold" name="minimumPaymentThreshold" type="number"/><Toggle label="Automatic eligibility" name="automaticEligibility"/><Toggle label="Manual payment approval" name="manualPaymentApproval"/><Field label="Payment instructions" name="paymentInstructions"/><h2 className="col-span-full text-lg font-black">Communication</h2><Toggle label="In-app notifications globally" name="appNotificationEnabled"/><Toggle label="Email globally" name="emailEnabled"/><Field label="Email retry count" name="emailRetryCount" type="number"/><Field label="Retry interval minutes" name="emailRetryIntervalMinutes" type="number"/><Field label="Communication retention days" name="communicationRetentionDays" type="number"/><label className="col-span-full text-sm font-bold">Milestone alert thresholds<input name="milestoneThresholds" defaultValue={(settings.milestoneThresholds || [settings.milestoneCount || 25]).join(",")} className="mt-1 w-full rounded-lg border p-2"/></label><button className="col-span-full w-fit rounded-lg bg-indigo-600 px-4 py-2 font-bold text-white">Save all affiliate settings</button></form>;
}

function AffiliateModal({ affiliate = {}, mode, onClose, onSubmit }) {
  const editing = mode === "edit";
  const fields = [["firstName", "First name"], ["lastName", "Last name"], ["affiliateName", "Affiliate name"], ["email", "Email"], ["mobile", "Mobile"], ["username", "Username"], ["referralTarget", "Referral target per cycle"], ["commissionRatePercent", "Commission rate %"], ["referralCodeActivatedAt", "Referral code start date"], ["referralCodeExpiresAt", "Referral code expiration date"], ["company", "Company"], ["profession", "Profession"], ["accountHolderName", "Account holder"], ["bankName", "Bank"], ["accountNumber", "Account number"], ["ifsc", "IFSC"], ["upiId", "UPI ID"], ["pan", "PAN"], ["gst", "GST"]];
  const inputType = (name) => name.includes("At") ? "date" : ["referralTarget", "commissionRatePercent"].includes(name) ? "number" : "text";
  const inputValue = (name) => name === "accountNumber" ? "" : name.includes("At") && affiliate[name] ? String(affiliate[name]).slice(0, 10) : affiliate[name] || "";
  return <Modal title={editing ? "Edit affiliate" : "Create affiliate"} onClose={onClose}><form onSubmit={onSubmit} className="grid gap-3 md:grid-cols-2">{fields.map(([name, label]) => <label key={name} className="text-sm font-bold">{label}<input name={name} type={inputType(name)} defaultValue={inputValue(name)} placeholder={name === "accountNumber" && editing ? "Enter only to update" : ""} required={!editing && ["affiliateName", "email", "username"].includes(name)} className="mt-1 w-full rounded-lg border p-2"/></label>)}{!editing && <label className="text-sm font-bold">Password<input name="password" type="password" required minLength={8} className="mt-1 w-full rounded-lg border p-2"/></label>}<label className="text-sm font-bold">Referral code status<Select name="referralCodeStatus" value={affiliate.referralCodeStatus || "ACTIVE"} options={["ACTIVE", "INACTIVE"]} /></label><label className="text-sm font-bold">Status<Select name="status" value={affiliate.status || "ACTIVE"} options={["ACTIVE", "INACTIVE", "SUSPENDED"]} /></label><div className="col-span-full flex justify-end gap-2"><button type="button" onClick={onClose} className="rounded-lg border px-4 py-2">Cancel</button><button className="rounded-lg bg-indigo-600 px-4 py-2 font-bold text-white">{editing ? "Save" : "Create"}</button></div></form></Modal>;
}
...
function DetailsPanel({ data, loading, onClose }) {
  ...
  return <div className="fixed inset-0 z-[100] flex justify-end bg-black/50"><aside className="h-full w-full max-w-4xl overflow-auto bg-white p-6 shadow-2xl"><div className="mb-4 flex justify-between gap-3"><h2 className="text-xl font-black">Affiliate Details</h2><button onClick={onClose} className="rounded-lg border px-3 py-1">Close</button></div>{loading || !item ? <p className="font-bold text-slate-500">Loading details...</p> : <div className="space-y-5"><div className="grid gap-4 md:grid-cols-4">{[["Affiliate name", item.affiliateName], ["Email", item.email], ["Phone", item.mobile || "-"], ["Account status", item.status], ["Joined", date(item.createdAt)], ["Referral code", item.affiliateCode], ["Code status", item.referralCodeStatus || "ACTIVE"], ["Code activated", date(item.referralCodeActivatedAt)], ["Code expires", item.referralCodeExpiresAt ? date(item.referralCodeExpiresAt) : "No expiry"], ["Commission config", `${item.commissionRatePercent ?? "Default"}%`], ["Total referrals", item.totalClicks], ["New installs", item.newAppInstallations], ["Registered users", item.registrations], ["Existing users", item.existingUserLogins], ["Successful subscriptions", item.successfulPurchases], ["Pending subscriptions", item.pendingConversions], ["Failed subscriptions", item.failedOrCancelledPurchases], ["Completed cycles", (data.cycles || []).filter((cycle) => Number(cycle.successfulReferralCount || 0) >= Number(cycle.targetCount || 0) && cycle.status !== "CANCELLED").length], ["Total commission", money(item.commissionEarned)], ["Pending commission", money(item.pendingCommission)], ["Paid commission", money(item.paidCommission)]].map(([label, value]) => <div key={label} className="rounded-xl border bg-slate-50 p-4"><p className="text-xs font-bold text-slate-500">{label}</p><p className="font-black">{value}</p></div>)}</div><div className="rounded-xl border bg-slate-50 p-4"><p className="text-xs font-bold text-slate-500">Referral link</p><p className="break-all font-black">{item.referralLink}</p></div><Table headers={["Click", "Campaign", "Type", "Conversion", "Commission"]} rows={(data.referrals || []).map((row) => [row.referralClickId, row.campaign || "-", userType(row), row.conversionStatus, money(row.commissionAmount)])} /><Table headers={["Plan", "Amount", "Commission", "Status", "Date"]} rows={(data.purchases || []).map((row) => [row.planId, money(row.amount), money(row.commissionAmount), row.commissionStatus || "PENDING", date(row.purchaseAt)])} /><Table headers={["Title", "Delivery", "Read", "Sent"]} rows={(data.notifications || []).map((row) => [row.title, row.appNotificationStatus, row.readAt ? "Read" : "Unread", date(row.createdAt)])} /></div>}</aside></div>;
}
